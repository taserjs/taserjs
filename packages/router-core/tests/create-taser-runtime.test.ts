...
describe('createTaserRuntime', () => {
        ...
        '/test': {
          GET: {
            ...
            route: {
              ...
              handlerMiddlewares: [],
              handler: (ctx: {
                cookies: { set: (n: string, v: string) => void }
              }) => {
  ...
  it('parses JSON body via Hono', async () => {
    ...
    const runtime = createTaserRuntime(manifest, () => ({}))
    const response = await runtime.fetch(new Request('http://localhost/echo', {
      method: 'POST',
  ...
  it('parses urlencoded body via Hono parseBody', async () => {
    ...
    const runtime = createTaserRuntime(manifest, () => ({}))
    const response = await runtime.fetch(new Request('http://localhost/form', {
      method: 'POST',
  ...
  describe('defineMiddleware Hono bridge integration', () => {
    it('runs Hono middleware in layout chain', async () => {
      ...
      const corsMiddleware = { handler: createTaserCompatHandler(async (c: Context, next: Next) => {
    ...
    it('runs Hono middleware in route chain', async () => {
      ...
      const jwtMiddleware = { handler: createTaserCompatHandler(async (c: Context, next: Next) => {
    ...
    it('chains multiple Hono middlewares', async () => {
      ...
      const loggerMiddleware = { handler: createTaserCompatHandler(async (c: Context, next: Next) => {
      ...
      const compressionMiddleware = { handler: createTaserCompatHandler(async (c: Context, next: Next) => {
