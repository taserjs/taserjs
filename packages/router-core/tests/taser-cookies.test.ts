...
describe('createTaserCookieJar defaults', () => {
  ...
  it('uses app defaults from createTaserApp cookies config', () => {
    const jar = createTaserCookieJar(null, undefined, { httpOnly: false })
    jar.set('session', 'abc')
    const response = jar.applyTo(new Response())
    const serialized = (response.headers.getSetCookie?.() ?? [response.headers.get('set-cookie')!]).join('; ')
    expect(serialized).not.toMatch(/HttpOnly/i)
    expect(serialized).toMatch(/SameSite=Lax/i)
  })

  it('per-call options override app defaults', () => {
    const jar = createTaserCookieJar(null, undefined, { httpOnly: true })
    jar.set('session', 'abc', { httpOnly: false })
    const response = jar.applyTo(new Response())
    const serialized = (response.headers.getSetCookie?.() ?? [response.headers.get('set-cookie')!]).join('; ')
    expect(serialized).not.toMatch(/HttpOnly/i)
  })
