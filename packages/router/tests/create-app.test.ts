...
describe('createContext + createTaserApp', () => {
  ...
  it('skips response validation when response.validate is false', async () => {
    ...
    const loose = createTaserApp({ response: { validate: false } }).context({}).create(manifest)
  ...
  it('creates app without context()', async () => {
    const route = createTaserApp().get('/hello').handler(() => reply.json({ ok: true }))
    const manifest = {
      layouts: {},
      routes: {
        '/hello': {
