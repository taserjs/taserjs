...
describe('returns fluent API', () => {
  ...
  it.each([
    {
      ...
      build: () => {
        ...
        return t.get('/hello').use(auth).returns({ 200: z.object({ ok: z.boolean() }) })
    ...
    },
    {
      ...
      build: () => {
        ...
        return t.get('/hello').returns({ 200: z.object({ id: z.string() }) }).handler(unit)
      },
      expectStatus: 404,
    },
    {
      ...
      build: () => {
        const auth = defineMiddleware({})
          .returns({ 401: z.object({ error: z.string() }) })
          .handler((_ctx, next) => next())
        return t.get('/hello').use(auth).returns({ 200: z.string() })
  ...
  it('auto-injects 422 when handler middleware defines input schemas', () => {
    ...
    const handler = defineHandler().use(unitMw).handler(() => reply.json({ ok: true }))
