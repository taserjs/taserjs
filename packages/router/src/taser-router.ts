...
export class TaserRouter<TAppContext extends Record<string, unknown> = AppContext> {
  ...
  >(definition: ContextDefinition<TBoot, TReq>): TaserRouter<TBoot & TReq> {
    this.state.contextDef = definition
    return this as unknown as TaserRouter<TBoot & TReq>
  }

  onError<TResponses extends import('./types/returns.js').ReturnsMap = import('./types/returns.js').ReturnsMap>(
  ...
  get: CreateWithoutBodyRoute<'GET', TAppContext> = createGetRoute as CreateWithoutBodyRoute<'GET', TAppContext>
  post: CreateWithBodyRoute<'POST', TAppContext> = createPostRoute as CreateWithBodyRoute<'POST', TAppContext>
  put: CreateWithBodyRoute<'PUT', TAppContext> = createPutRoute as CreateWithBodyRoute<'PUT', TAppContext>
  patch: CreateWithBodyRoute<'PATCH', TAppContext> = createPatchRoute as CreateWithBodyRoute<'PATCH', TAppContext>
  delete: CreateWithoutBodyRoute<'DELETE', TAppContext> = createDeleteRoute as CreateWithoutBodyRoute<'DELETE', TAppContext>
  options: CreateWithoutBodyRoute<'OPTIONS', TAppContext> = createOptionsRoute as CreateWithoutBodyRoute<'OPTIONS', TAppContext>
  head: CreateWithoutBodyRoute<'HEAD', TAppContext> = createHeadRoute as CreateWithoutBodyRoute<'HEAD', TAppContext>
  ...
  create<const TManifest extends RouteManifestShape>(manifest: TManifest): TaserApp<TManifest> {
    ...
    const bootPromise = Promise.resolve(
      ...
      definition.boot?.() ?? ({} as Record<string, unknown>),
    )
