...
function watchGeneratorOptions(configDir: string, routesDir: string, outputFile: string, extra = {}) {
  return {
...
describe('watch integration', () => {
  ...
  it('skips regeneration when file mtime is unchanged', async () => {
    ...
    const generator = new Generator(watchGeneratorOptions(configDir, routesDir, outputFile, { quiet: true }))
    generator.enableWatchMode()
    await generator.run()
