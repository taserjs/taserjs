...
describe('FileIndex and IncrementalRouteModel', () => {
  it('updates a single route without full rescan', async () => {
    ...
    await model.applyFileUpsert(routesDir, outputFile, join(routesDir, 'posts.get.ts'), scanOptions)
  ...
  it('recomputes layout chains for routes under a changed layout', async () => {
    ...
    const overview = model.toGeneratedModel().routes.find(route => route.urlPath === '/account/overview')
