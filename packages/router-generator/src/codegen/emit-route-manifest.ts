  ...
  for (const layoutId of model.layoutIds) {
    ...
    properties.push(
      objectProperty(
        str(layoutId),
        objectExpression([
          objectProperty(id('middlewares'), id(layout.importName)),
        ]),
      ),
    )
  }
...
function buildManifestRoutesObject(model: GeneratedModel): TSESTree.ObjectExpression {
  ...
  for (const urlPath of sortedPaths) {
    ...
    properties.push(
      objectProperty(
        str(urlPath),
        objectExpression(methodProperties),
      ),
    )
  }
...
function buildProgram(model: GeneratedModel): TSESTree.Program {
  ...
  const layoutTreeType = buildLayoutTreeType(
    model.layoutIds,
    model.layoutParents,
    model.layouts,
  )
...
export function emitRouteManifestSource(model: GeneratedModel, options: EmitManifestOptions): string {
