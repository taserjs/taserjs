...
export class Generator {
  ...
  private async runInternal(): Promise<GeneratorRunResult> {
    ...
    const writeResult = await writeManifestIfChanged(config.outputFile, source, { force: this.force })
  ...
  private async tryPersistentCacheSkip(): Promise<boolean> {
    ...
    if (!PersistentCache.matchesFileIndex(cache, config.routesDir, config.outputFile, index.toMtimeRecord())) {
      return false
    }
    ...
    this.logger.info(`Skipped generation; route files unchanged (${relative(process.cwd(), config.outputFile)})`)
    return true
  }
}
