...
import { Generator, type GeneratorEventType, type WatchRouteTreeHandle } from '../generator/generator.js'
...
export async function watchRouteTreeInternal(options: GeneratorRunOptions): Promise<WatchRouteTreeHandle> {
...
async function handleWatchEvent(
  ...
  try {
    ...
    logger.error(`Route tree generation failed: ${error instanceof Error ? error.message : String(error)}`)
  }
}
