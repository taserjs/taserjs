...
export const Route = createFileRoute('/sponsor')({
  ...
  head: () => ({
    meta: openGraphMeta({
      ...sitePages.sponsor,
      image: getSiteOgImageUrl('sponsor'),
    }),
  }),
})
...
function SponsorPage() {
  return (
    <HomeLayout {...baseOptions()}>
      <PageHero
        eyebrow="Sponsorship"
        title={<>Sponsor <SectionAccent>Taser</SectionAccent></>}
        ...
        actions={(
          <>
            ...
            <Link to="/motivation" className={secondaryButtonClass}>
              Read the motivation
            </Link>
          </>
        )}
      ...
      <PageSection
        title={<>A note from the <SectionAccent>maintainers</SectionAccent></>}
      ...
      >
        <CalloutCard icon={<Heart aria-hidden />}>
          We started Taser after seeing the same pattern repeat: demos look great, but real APIs need composition,
          validation, and shared context. Sponsorship helps us keep improving the core router, adapters, codegen,
          and typed client so you can focus on product work instead of plumbing.
        </CalloutCard>
      </PageSection>

      <PageSection
        muted
        title={<>What your sponsorship <SectionAccent>supports</SectionAccent></>}
        description="This is where sponsor funds go. The goal is simple: keep Taser dependable and easy to adopt."
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <InfoCard icon={<ShieldCheck aria-hidden />} iconTone="emerald" title="Quality and stability">
            <CheckList
              items={[
        ...
        <CalloutCard className="mt-8">
          If you want a specific feature, integration, or migration help, include it in the sponsor note or email us.
