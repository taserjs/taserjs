...
<Tabs items={["Before (Express)", "After (Taser.js)"]}>
  ...
  <Tab value="After (Taser.js)">
    ...
    *The path string above is automatically populated by the dev watcher, production builds (`vite build`, `next build`), or `@taserjs/router-cli generate` when the file is first created—you don't need to author it by hand even during manual migration.*
