...
Before the route handler executes, Taser.js validates `params`, `query`, `headers`, and `body` against their schemas. If validation fails, a `ValidationError` is raised and converted to a built-in **422** response. It does **not** reach `defineTaser().onError()` — customize validation envelopes in middleware `try/catch` instead.
...
| **Declaration Type**    | Universal type parameter in `defineTaser().context(...)`         | Inferred automatically from `return next({ ... })` in layouts            |
