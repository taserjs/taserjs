...
<Callout type="warn" title="Path Traversal Protection">
  When `root` is specified, Taser.js validates that the resolved file path does not escape the
  boundary directory, protecting your application against `../` directory traversal attacks.
</Callout>
