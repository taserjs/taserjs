...
export default t
  ...
  .handler(async ({ ctx, state }) => {
    const user = await ctx.db.getCurrentUser(state.userId);
...
`@taserjs/client` types `await res.json()` automatically from your route handlers — **you do not need `.returns()` for client type safety**. By default, the client unions successful `ReplyOf` payload types (`200`–`226`) from reply helpers like `json()`, `ok()`, and `created()`.
