---
title: Reply Helpers
description: "Send clean, status-discriminated HTTP responses using tree-shakeable reply helpers: json(), ok(), notFound(), and redirect()."
icon: MessageSquare
---
...
export default t.delete("/items/:id").handler(async ({ ctx, req }) => {
  await ctx.db.deleteItem(req.params.id);
  return noContent();
});
```
...
| `methodNotAllowed(data?, init?)`    | `405 Method Not Allowed`    | HTTP method not supported for this route |
...
| `unprocessable(data?, init?)`       | `422 Unprocessable Entity`  | Schema validation error                  |
...
export default t.get("/documents/:id").handler(async ({ ctx, req, state }) => {
  const user = state.user;
  if (!user) {
    return unauthorized({ message: "Login required" });
  }

  const doc = await ctx.db.findDocument(req.params.id);
  if (!doc) {
...
Prefer the first-party `cookie()` middleware and `{ cookies }` jar (see [Cookie Management](/docs/responses/cookies)). You can also set `Set-Cookie` manually via response init:

```ts title="src/routes/auth/login.post.ts"
import { json } from "@taserjs/router/reply";
import { t } from "@taserjs/router";

export default t.post("/auth/login").handler(async () => {
...
<Cards>
  ...
  <Card
    ...
    description="Stream Web ReadableStreams, binary payloads, and SSE."
  />
</Cards>
