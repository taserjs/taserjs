...
When writing reusable standalone middlewares that assume certain state was already produced upstream (e.g. an authenticated user or tenant ID), declare a state requirement via `.requires<TState>()` on `middleware()`:
