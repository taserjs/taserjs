...
<Callout type="info" title="Explicit End-to-End Type Safety">
  Pass `AppManifest` from `routes.gen.ts` as the generic to `createClient<AppManifest>()`. The client
  infers routes, methods, input schemas, and return contracts without generating static client source files.
</Callout>
