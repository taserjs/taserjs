...
The `@taserjs/client` package provides a lightweight, zero-codegen proxy client that infers end-to-end type safety directly from your server's router type definition (`RouteManifest` or `typeof app`).
...
import type { RouteManifest } from "path/to/.taser/types/routes.js";
// Or when using vite based projects:
// import type { RouteManifest } from "#taserjs/virtual/manifest";
// Or on Next.js with the Taser.js adapter:
