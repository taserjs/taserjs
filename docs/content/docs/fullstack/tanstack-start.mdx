...
<Steps>
  ...
  <Step>
    ...
    Update `package.json` and `tsconfig.json` so both your server routes and frontend code can import `#taserjs/router` and resolve generated ambient types in `.taser/types/`:

    ```json title="package.json"
    {
      "imports": {
        "#taserjs/router": "./src/server/taser.ts"
      }
    }
    ```
...
// Initialize client with your app's base URL
