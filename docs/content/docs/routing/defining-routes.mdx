...
<Callout type="info" title="JSON & Multipart / File Uploads">
  The `.body()` method validates `application/json` by default and also supports `form`, `text`, and `raw` modes. See the [Standard Schema Validation Guide](/docs/validation/standard-schema#validating-multipart-form-data--file-uploads) for examples on validating `File` uploads, sizes, and MIME types.
</Callout>
