...
- **Mismatched Path String Drift**: If you provide a path string that exists elsewhere in your project (e.g. calling `t.get("/users/:id")` inside `src/routes/articles.get.ts`), runtime routing still dispatches strictly to `/articles`, but `ctx.params` will drift to reflect `{ id: string }` instead of `{}`.
