...
- **Compile-Time Type Checking**: Ambient types emitted by `@taserjs/cli` or Vite augment `RouterRegister.RoutePath`. Passing an unknown or misspelled route path causes TypeScript (`tsc`) to report a compile error:
