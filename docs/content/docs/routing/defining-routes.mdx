...
- **Compile-Time Type Checking**: Ambient types emitted by `@taserjs/router-cli` or Vite augment `RouterRegister.RoutePath`. Passing an unknown or misspelled route path causes TypeScript (`tsc`) to report a compile error:
