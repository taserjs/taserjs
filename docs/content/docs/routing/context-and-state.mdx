...
const taser = defineTaser().context(context);
...
async function getUserProfile({ ctx, req }: RouteContext) {
...
export default GET.handler(async (args) => {
  const user = await getUserProfile(args);
  if (!user) {
    return notFound({ message: "User not found" });
  }
  return json(user);
...
<Cards>
  <Card
    ...
    description="Mount cookie() middleware and use the Cookie Jar Instance in handlers."
  />
  <Card
