...
| **TypeScript Typecheck** | Emits ambient `RoutePath` / `LayoutId` types            | Validated by `tsc` against ambient `RoutePath`; types `req.params`         |
...
Inside your route handler, parameters are automatically typed and validated on `req.params`:
...
export default t
  ...
  .handler(({ req }) => {
    // req.params.id is strictly typed as a UUID string
    return json({ taskId: req.params.id, completed: true });
  });
```
